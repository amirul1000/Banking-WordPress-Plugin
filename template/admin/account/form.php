...
?>
<a  href="<?php echo 'admin.php?page=account'; ?>" class="btn btn-info"><i class="arrow_left"></i> List</a>
<h5 class="font-20 mt-15 mb-1"><?php if($id<0){echo "Save";}else { echo "Update";} echo " "; echo str_replace('_',' ','Account'); ?></h5>
<!--Form to save data-->
<form method="post" action="admin.php?page=account&cmd=save&id=<?=$account[0]->id?>" enctype="multipart/form-data">
   <div class="row"> 
      <div class="col">
          ...
          <div class="col-md-8"> 
            <select name="users_id"  id="users_id"  class="form-control"/> 
             <option value="">--Select--</option> 
			  ...
			  ?>
              <option value="<?=$user->ID?>" <?php if($account[0]->users_id==$user->ID){ echo "selected";} ?>><?=$user->first_name?> <?=$user->last_name?> <?=$user->user_email?></option> 
              <?php
				}
			  ?>	
            </select> 
