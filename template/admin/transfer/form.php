...
?>
<a  href="<?php echo 'admin.php?page=transfer'; ?>" class="btn btn-info"><i class="arrow_left"></i> List</a>
<h5 class="font-20 mt-15 mb-1"><?php if($id<0){echo "Save";}else { echo "Update";} echo " "; echo str_replace('_',' ','Transfer'); ?></h5>
<!--Form to save data-->
<form method="post" action="admin.php?page=transfer&cmd=save&id=<?=$transfer[0]->id?>" enctype="multipart/form-data">
   <div class="row"> 
      <div class="col">
         ...
         <label for="users_id" class="col-md-8 control-label">Member</label> 
          <div class="col-md-8"> 
            <select name="users_id"  id="users_id"  class="form-control"/> 
             <option value="">--Select--</option> 
			  ...
			  ?>
              <option value="<?=$user->ID?>" <?php if($transfer[0]->users_id==$user->ID){ echo "selected";} ?>><?=$user->first_name?> <?=$user->last_name?> <?=$user->user_email?></option> 
              <?php
				}
			  ?>	
            </select> 
          ...
          <div class="col-md-8"> 
           ...
           <div class="form-group"> 
          <label for="bank_routing_number" class="col-md-8 control-label">Transfer status</label> 
          <div class="col-md-8"> 
            <select name="transfer_status"  id="transfer_status"  class="form-control"/> 
             <option value="">--Select--</option> 
              <option value="pending" <?php if($transfer[0]->transfer_status=='pending'){ echo "selected";} ?>>pending</option> 
              <option value="completed" <?php if($transfer[0]->transfer_status=='completed'){ echo "selected";} ?>>completed</option> 
              <option value="rejected" <?php if($transfer[0]->transfer_status=='rejected'){ echo "selected";} ?>>rejected</option> 
            </select> 
          </div> 
           </div>
